...
SELECT_CATEGORY, ENTER_DESCRIPTION, UPLOAD_PHOTO, CONFIRM_ORDER = range(4)
...
class OrderHandler:
    ...
    async def select_category(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        category_map = {
            "cat_repair": "Ремонт одежды",
            "cat_sewing": "Пошив",
            "cat_design": "Декор и дизайн",
            "cat_fitting": "Подгонка по фигуре"
        }

        context.user_data['order_category'] = category_map.get(query.data, "Другое")
        
        await query.edit_message_text(
            f"✅ Выбрана категория: {context.user_data['order_category']}\n\n"
            "📝 Теперь опишите детали вашего заказа:",
    ...
    async def enter_description(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        await update.message.reply_text(
            "📸 Отлично! Теперь отправьте фото (или нажмите 'Пропустить'):",
