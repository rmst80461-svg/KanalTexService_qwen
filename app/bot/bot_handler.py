...
class TelegramBot:
    ...
    async def cmd_start(self, update: Update, context):
        ...
        welcome_text = (
            ...
            f"Меня зовут <b>Аква</b> — я ваш помощник в <b>КаналТехСервис</b>.\n\n"
            f"🚚 Откачка септиков • Прочистка канализации\n"
            f"💧 Каналопромывка • Илосос • Диагностика\n\n"
            f"⏰ <b>Работаем 24/7</b> — выезд в течение часа!\n"
            f"📍 г. Ярцево и Смоленская область"
        )
        ...
        if user_id not in self.admin_ids:
            await update.message.reply_text(
                "👇 <b>Чем могу помочь?</b> Выберите нужное:",
                parse_mode=ParseMode.HTML,
    ...
    async def handle_text_input(self, update: Update, context):
        ...
        if step == 'enter_address':
            ...
            await update.message.reply_text(
                "📞 Отлично! Теперь укажите номер телефона для связи:",
                parse_mode=ParseMode.HTML
            )
            ...
            await update.message.reply_text(
                "💬 Почти готово! Добавьте комментарий или напишите «нет»:",
                parse_mode=ParseMode.HTML
            )
            ...
            await update.message.reply_text(
                ...
                f"Спасибо, что выбрали <b>КаналТехСервис</b>! 💧",
                parse_mode=ParseMode.HTML,
                reply_markup=get_main_menu()
            )
            ...
            await update.message.reply_text(
                "🤔 Не понял вас. Нажмите кнопку меню или выберите действие:",
                reply_markup=get_main_menu()
            )
        ...
        try:
            ...
            # Обработка услуг для заказа
                ...
                if service == "other":
                    ...
                    await query.message.reply_text(
                        "👋 Здравствуйте! Меня зовут <b>Аква</b>.\n\n"
                        "Опишите, пожалуйста, вашу проблему или задайте вопрос — "
                        "с радостью помогу разобраться!\n\n"
                        "💡 Отвечу на вопросы об услугах, ценах и сроках работ.",
                        parse_mode=ParseMode.HTML,
                        reply_markup=get_back_button()
                    )
                else:
                    ...
                    await query.message.reply_text(
                        ...
                        f"📍 Укажите адрес, куда приехать мастеру:",
                        parse_mode=ParseMode.HTML
                    )
